...
def set_proxied_state_for_dag_if_changed(
    ...
    if get_local_proxied_state_dir():
        logger.info(
            "Executing in local mode. Not setting proxied state in airflow metadata database, and instead expect dagster to be pointed at proxied state via DAGSTER_AIRLIFT_MIGRATION_STATE_DIR env var."
        )
        return
    else:
